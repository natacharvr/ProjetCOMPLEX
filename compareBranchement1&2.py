...
[time_branchement, time_branchement2] = lectureFichier("benhmark/ComparaisonBranchementsAmeliorés.txt", 88)

fig, ax = plt.subplots()
cmap = plt.get_cmap("hot", 4) 
# plt.xlim(0.1,229)
# plt.ylim(0.1,50)
ax.set_title("Comparaison des temps d'exécution des algorithmes de\n branchements améliorés 1 et 2 pour p=0.2", pad=10)
ax.plot([i for i in range(1,89)], time_branchement, label="Version 1", color=cmap(1))
ax.plot([i for i in range(1,89)], time_branchement2, label="Version 2", color=cmap(2))
...
plt.savefig('plots/compareAmeliorés.png', dpi=300, bbox_inches='tight')
plt.show()
